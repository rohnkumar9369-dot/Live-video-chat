...
const Dashboard = () => {
  ...
  // Instant photo update ke liye local state
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // Merge: true lagaya taaki error na aaye
  ...
  const handlePhotoChange = async (e) => {
    ...
    // 1. Select karte hi instant screen par dikhane ka logic (Superfast feel)
    ...
    try {
      // 2. Date.now() lagaya taaki browser purani photo yaad na rakhe
      ...
      setLocalPhoto(user?.photoURL || "https://via.placeholder.com/150") // Error aane par purani photo wapas
    } finally {
      setUploading(false)
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans relative">
      ...
      <main className="flex-1 p-4 flex flex-col items-center overflow-y-auto pb-10">
        ...
        <div className="w-full max-w-md bg-white p-8 rounded-3xl shadow-md border border-gray-100 text-center mt-6 mb-8 flex flex-col items-center">
          ...
          <button onClick={() => navigate('/call')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-2xl shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2">
              ...
              {messages.length > 0 ? messages.map((msg) => (
                <div key={msg.id} className="bg-gray-50 p-2 rounded-xl flex items-center gap-2 border border-gray-100">
                   ...
                   <div className="flex-1 overflow-hidden">
                     ...
                     <p className="text-[10px] text-gray-400 truncate">{msg.timestamp?.seconds ? new Date(msg.timestamp.toDate()).toLocaleTimeString() : 'Just now'}</p>
                   </div>
                </div>
              )) : (
              ...
              {calls.length > 0 ? calls.map((call) => (
                <div key={call.id} className="bg-gray-50 p-2 rounded-xl flex items-center gap-2 border border-gray-100">
                   ...
                   <div className="flex-1 overflow-hidden">
                     ...
                     <p className="text-[10px] text-gray-400 truncate">{call.timestamp?.seconds ? new Date(call.timestamp.toDate()).toLocaleTimeString() : 'Just now'}</p>
                   </div>
                </div>
              )) : (
          ...
          <div className="w-full max-w-sm bg-white rounded-3xl shadow-2xl flex flex-col gap-5" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex flex-col items-center gap-4 px-5">
              <div className="relative">
                {/* Yahan localPhoto lagaya hai taaki instant dikhe */}
              ...
              <div className="w-full bg-gray-50 p-3 rounded-xl border border-gray-100">
                <label className="text-[10px] text-gray-400 uppercase font-bold tracking-wider block mb-1">Display Name</label>
                {editing ? (
                  <div className="flex gap-2">
                    {/* NAAM WALA DIBBA: Yahan text-black aur bg-white daal diya hai! */}
                    <input 
                      value={newName} 
                      ...
                      autoFocus
                    />
                    <button onClick={handleUpdate} className="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-md text-xs font-bold shadow-sm transition">Save</button>
                  </div>
                ) : (
                  <div className="flex items-center justify-between bg-white px-3 py-2 rounded-md border border-gray-200">
                    ...
                    <Edit2 size={14} className="text-gray-400 cursor-pointer hover:text-blue-600 transition" onClick={() => setEditing(true)} />
                  </div>
                )}
              </div>
              ...
              <div className="w-full bg-gray-50 p-3 rounded-xl flex justify-between items-center border border-gray-100">
                ...
                <button onClick={() => { navigator.clipboard.writeText(user.uid); toast.success("Copied!"); }} className="p-2 hover:bg-gray-200 rounded-full transition text-gray-500">
                  <Copy size={14} />
                </button>
              </div>
